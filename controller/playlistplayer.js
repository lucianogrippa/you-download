...
function loadSrc(firstMedia) {
  if (!!firstMedia) {
    ...
    $("#videoTitle").html(
      formatTitle(firstMedia.name) +
        "   (" +
        type +
        " " +
        getExt(firstMedia.name) +
        ")"
    );
    player.load();
    player
...
function buldPlaylistView(playlist) {
  if ($(".prows").length > 0) $(".table-playlist .prows").remove();
      ...
      btnDelete.on("click", function (event) {
        ...
        if (conf) {
          try {
            ...
            alert("Error while deleting file " + path);
            Log.error(e);
          }
          loadFilesInDir();
        }
